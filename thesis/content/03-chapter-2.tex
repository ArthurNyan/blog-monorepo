...
Длинная таблица~\ref{tab:ch2-long} иллюстрирует работу \texttt{longtable} при переносе
на несколько страниц и аккуратное выравнивание чисел:
\begin{longtable}{@{}c>{\raggedright\arraybackslash}p{0.48\linewidth}S[table-format=2.2]S[table-format=1.2]@{}}
...
\begin{figure}[tbp]
  ...
  \caption{Архитектура кэширования запросов в распределенном сервисе}\label{fig:ch2-cache}
\end{figure}
...
Пример команды сборки приведен в листинге~\ref{lst:ch2-make-target}:
...
\begin{algorithm}[tbp]
  \caption{Формирование отчета контроля сборки}\label{alg:ch2-report}
  \begin{algorithmic}[1]
    \Require Массив \texttt{checks}, содержащий статусы проверок
    \Ensure Текстовый отчет \texttt{report}
    \State \texttt{passed} $\gets$ 0
    \State \texttt{report} $\gets$ пустой список
    \ForAll{элемент \texttt{check} в \texttt{checks}}
      \If{\texttt{check.status} = ``ok''}
        \State \texttt{passed} $\gets$ \texttt{passed} + 1
        \State добавить строку ``OK: check.name'' в \texttt{report}
      \Else
        \State добавить строку ``FAIL: check.name'' в \texttt{report}
      \EndIf
    \EndFor
    \State добавить итог ``passed / |checks|'' в \texttt{report}
    \State \Return \texttt{report}
  \end{algorithmic}
\end{algorithm}
...
\texttt{listings}, \texttt{algorithmicx} и \texttt{siunitx}~\cite{listings-doc,algpseudocode-doc,siunitx-doc},
