...
\begin{enumerate}
  ...
  \item реализован безопасный режим \texttt{preview} для \texttt{home-page}, \texttt{page},
  \texttt{article}, \texttt{project}, \texttt{vacancy};
  \item реализован \texttt{SEO/Open Graph}-контур для ключевых сущностей и генерация
  \texttt{sitemap};
  \item реализованы серверные маршруты для форм \texttt{lead-submission} и
...
\begin{itemize}
  ...
  \item размер клиентского артефакта сборки составляет около \texttt{2.4 МБ}.
\end{itemize}
...
\begin{enumerate}
  \item расширение англоязычного наполнения детальных разделов
  \texttt{articles/projects};
  \item дальнейшее развитие \texttt{SEO}-контура в \texttt{CMS} и углубление редакторской настройки
  публичных страниц;
  \item расширение браузерного контура оценки доступности и производительности.
\end{enumerate}
