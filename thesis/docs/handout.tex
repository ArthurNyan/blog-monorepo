...
\hline
Основной frontend стек & \texttt{Astro 6}, \texttt{React}, \texttt{Tailwind}, \texttt{Vercel adapter}, \texttt{sitemap} \\
\hline
...
По состоянию текущего baseline подтверждены следующие результаты:
\begin{enumerate}
...
Для проекта подготовлена воспроизводимая матрица приемки. На baseline от \texttt{22.05.2026}
подтверждено:
\begin{itemize}
...
\begin{enumerate}
  \item в результате выполнения выпускной квалификационной работы разработан прототип
  \texttt{CMS-first} системы управления контентом для корпоративного маркетингового сайта;
