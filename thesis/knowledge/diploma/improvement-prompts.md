...
- `P0-1` закрыт по новой publish-ready структуре главы 1;
- ниже он сохранен только как краткая history note, а не как активный prompt для
  повторного запуска;
- для новых agent-тредов по структуре главы 1 источниками истины считать
  `thesis-structure.md`, `final-thesis-outline.md`, `agent-rules.md` и
  `decision-log.md`.
...
1. `P0-2` Синхронизировать `SEO`-раздел второй главы.
2. `P1-1` Усилить доказательство `Dokploy`-контура.
3. `P1-2` Закрыть или формально ограничить мультиязычность карьерного модуля.
4. `P1-3` Вычистить хвосты `Vercel` и устаревший demo-контент.
5. `P1-4` Пересобрать evidence baseline.
6. `P2-1` Выровнять `Nx`-targets и фактический test contour.
7. `P2-2` Добавить подраздел про эксплуатационные риски.
8. `P2-3` Снять LaTeX warnings и привести сборку к чистому `make check`.

---

## `P0-1` History Note: publish-ready переделка главы 1

Статус: выполнено и синхронизировано `2026-06-06`.

Что было зафиксировано:

- активная структура главы 1 приведена к схеме `1.1` предметная область,
  `1.2` подходы к управлению контентом, `1.3` выбор `Strapi` и `Astro`,
  `1.4` формирование требований к системе;
- самостоятельный baseline-блок из publish-ready guidance убран;
- проектный контекст для главы 1 допустим только как короткое описание авторской
  разработки или архитектурных предпосылок решения;
- будущие agent-треды не должны возвращать старую структуру первой главы.

Где смотреть актуальную guidance-базу:
...
Зачем этот пункт оставлен:

- как краткая history note о закрытом structural change;
- как напоминание не возвращать в active prompts старую логику главы 1.
